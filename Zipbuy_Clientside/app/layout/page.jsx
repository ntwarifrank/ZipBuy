...
const Layout = ({children}) => {
    ...
    <>
      <div className="px-2 sm:px-3 flex flex-row justify-between items-center">
        <div className="flex items-center">
          <p className={`font-bold text-alibabaOrange text-xl ${dashboardState && !isMobileView ? "hidden" : "block"}`}>ZIPBUY</p>
        </div>
        <div>
          {isMobileView ? (
            <X 
      ...
      <div className="flex flex-col justify-between h-full pb-12 mt-2">
        <div>
          <ul className="px-1 sm:px-2 py-3 sm:py-4 flex flex-col gap-2 sm:gap-3 font-semibold">
            <li className="flex flex-row gap-1 font-bold hover:text-alibabaOrange cursor-pointer transition-colors duration-200">
              <Link href="/buyingpage" className="cursor-pointer flex flex-row gap-1 items-center w-full" onClick={isMobileView ? closeMobileSidebar : undefined}>
                ...
                <span className={`${dashboardState && !isMobileView ? "hidden" : "block"} text-lg sm:text-sm whitespace-nowrap`}>Discover</span>
              </Link>
            </li>
  ...
  return (
    <div>
      <div className={mode ? "flex flex-row bg-mainColor text-gray-500 min-h-screen mb-0" : "flex flex-row bg-darkMode text-gray-400 min-h-screen mb-0"}>
